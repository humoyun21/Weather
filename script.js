...
darcmodBtn.addEventListener("click", () => {
  ...
  if (darcmodBtn.classList.contains("darcmod--btn--darc")) {
    ...
    darcCards.forEach(
      (el) => (el.style.backgroundColor = "rgba(217, 217, 217, 1)")
    );
    ...
    darcBtnWrap.style.backgroundColor = "rgba(217, 217, 217, 1)";
    darcCards.forEach(
      (el) => (el.style.backgroundColor = "rgba(68, 68, 68, 1)")
    );
...
async function getPost(cityName = "Tashkent") {
  try {
    ...
    windSpeed.textContent = `${Number.parseInt(
...
async function getPstDays(cityName = "Tashkent", day = 5) {
  try {
    let respons = await fetch(
      `${URL}/forecast.json?key=${key}&days=${day}&q=${cityName}`
    );
    let result = await respons.json();
    iconDay1.src = `${result.forecast.forecastday[0].day.condition.icon}`;
    cenciDay1.textContent = `${Number.parseInt(
    ...
    cenciDay2.textContent = `${Number.parseInt(
    ...
    cenciDay3.textContent = `${Number.parseInt(
    ...
    cenciDay4.textContent = `${Number.parseInt(
    ...
    cenciDay5.textContent = `${Number.parseInt(
...
async function getPossTime(cityName = "Tashkent", day = 1) {
  try {
    ...
    let result = await respons.json();
    //---------04:00-------
    dayTaim1.textContent = `${result.forecast.forecastday[0].hour[4].time.slice(
      11,
      16
    )}`;
    dayPagodImg1.src = `${result.forecast.forecastday[0].hour[4].condition.icon}`;
    dayC1.textContent = `${Number.parseInt(
      +result.forecast.forecastday[0].hour[4].temp_c
    )} °C`;
    dayImg1.style.cssText = `transform: rotate(${result.forecast.forecastday[0].hour[4].wind_degree}deg);`;
    dayWind1.textContent = `${Number.parseInt(
      +result.forecast.forecastday[0].hour[4].wind_kph
    )}km/h`;

    //---------08:00--
    dayTaim2.textContent = `${result.forecast.forecastday[0].hour[8].time.slice(
      11,
      16
    )}`;
    dayPagodImg2.src = `${result.forecast.forecastday[0].hour[8].condition.icon}`;
    dayC2.textContent = `${Number.parseInt(
      +result.forecast.forecastday[0].hour[8].temp_c
    )} °C`;
    dayImg2.style.cssText = `transform: rotate(${result.forecast.forecastday[0].hour[8].wind_degree}deg);`;
    dayWind2.textContent = `${Number.parseInt(
      +result.forecast.forecastday[0].hour[8].wind_kph
    )}km/h`;

    //---------12:00--------
    dayTaim3.textContent = `${result.forecast.forecastday[0].hour[12].time.slice(
      11,
      16
    )}`;
    dayPagodImg3.src = `${result.forecast.forecastday[0].hour[12].condition.icon}`;
    dayC3.textContent = `${Number.parseInt(
    ...
    dayWind3.textContent = `${Number.parseInt(
      +result.forecast.forecastday[0].hour[12].wind_kph
    )}km/h`;

    //---------16:00-------
    dayTaim4.textContent = `${result.forecast.forecastday[0].hour[16].time.slice(
      11,
      16
    )}`;
    dayPagodImg4.src = `${result.forecast.forecastday[0].hour[16].condition.icon}`;
    dayC4.textContent = `${Number.parseInt(
      +result.forecast.forecastday[0].hour[16].temp_c
    )} °C`;
    dayImg4.style.cssText = `transform: rotate(${result.forecast.forecastday[0].hour[16].wind_degree}deg);`;
    dayWind4.textContent = `${Number.parseInt(
      +result.forecast.forecastday[0].hour[16].wind_kph
    )}km/h`;

    //---------20:00--------
    dayTaim5.textContent = `${result.forecast.forecastday[0].hour[20].time.slice(
      11,
      16
    )}`;
    dayPagodImg5.src = `${result.forecast.forecastday[0].hour[20].condition.icon}`;
    dayC5.textContent = `${Number.parseInt(
      +result.forecast.forecastday[0].hour[20].temp_c
    )} °C`;
    dayImg5.style.cssText = `transform: rotate(${result.forecast.forecastday[0].hour[20].wind_degree}deg);`;
    dayWind5.textContent = `${Number.parseInt(
      +result.forecast.forecastday[0].hour[20].wind_kph
    )}km/h`;

    //---------00:00---------
    dayTaim6.textContent = `${result.forecast.forecastday[0].hour[0].time.slice(
      11,
      16
    )}`;
    dayPagodImg6.src = `${result.forecast.forecastday[0].hour[0].condition.icon}`;
    dayC6.textContent = `${Number.parseInt(
      +result.forecast.forecastday[0].hour[0].temp_c
    )} °C`;
    dayImg6.style.cssText = `transform: rotate(${result.forecast.forecastday[0].hour[0].wind_degree}deg);`;
    dayWind6.textContent = `${Number.parseInt(
      +result.forecast.forecastday[0].hour[0].wind_kph
    )}km/h`;
  } catch (err) {
    alert(err);
  }
